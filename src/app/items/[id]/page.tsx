    ...
    <div className="flex min-h-screen w-full flex-col bg-background pb-32">
      ...
      <main className="container mx-auto max-w-2xl px-6 pt-10">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            <Badge variant="outline" className="rounded-lg border-primary/30 text-primary font-bold">{(t.categories as any)[item.category] || item.category}</Badge>
      ...
      <div className="fixed bottom-8 left-1/2 z-50 w-full max-w-md -translate-x-1/2 px-4">
        <div className="flex h-20 items-center gap-3 rounded-[2.5rem] bg-foreground/95 p-3 shadow-2xl backdrop-blur-xl ring-1 ring-white/10">
          <Button variant="ghost" className="h-14 w-14 rounded-[1.5rem] bg-white/10 text-white hover:bg-white/20"><MessageCircle className="h-6 w-6" /></Button>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="rounded-[2.5rem] border-none bg-white p-8">
              ...
              <DialogFooter>
                <Button onClick={handleSendRequest} className="h-14 w-full rounded-2xl bg-primary text-foreground font-black shadow-xl">{isCoupon ? t.item.getCoupon : t.item.swapButton}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
